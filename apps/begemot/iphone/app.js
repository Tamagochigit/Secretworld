...
(() => {
 ...
 function view(v,scroll=true){
  if(!['home','check','network','chat','us','install'].includes(v))return;
  currentView=v;
  for(const name of ['home','check','network','chat','us','install'])$('#'+name+'Panel').hidden=name!==v;
  document.querySelectorAll('.tabs [data-view]').forEach(b=>{b.classList.toggle('selected',b.dataset.view===v);if(b.dataset.view===v)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});
 ...
 const help='<ol><li><strong>Прислали странное?</strong> Вставьте текст в «Проверку». Разбор выполняется на устройстве; ссылку Бегемот не открывает.</li><li><strong>Нужен разговор?</strong> Во вкладке «Разговор» работают локальные подсказки. Для свободного ИИ подключите свой ключ, если готовы передать ему вопрос.</li><li><strong>Хотите значок?</strong> В Safari нажмите «Поделиться» → «На экран Домой». Устанавливать приложение не обязательно.</li></ol><p>Бегемот помогает заметить признаки, но не подтверждает безопасность отправителя или сайта.</p>';
 const privacy='<p><strong>Локальная проверка.</strong> Текст анализируется в браузере. Он не отправляется Secretworld и не попадает в архив.</p><p><strong>Онлайн-ИИ.</strong> Только после настройки ключа и нажатия «Отправить в ИИ» ключ передаётся в OpenRouter для запроса, а вопрос — OpenRouter и провайдеру модели. Ключ остаётся в памяти страницы и очищается при закрытии. Недавний контекст и заметки по умолчанию выключены; их можно включить в «Памяти».</p><p><strong>На устройстве.</strong> При включении сохраняется история разговора и заметки в хранилище Safari. Они не отправляются Secretworld. Очистка данных сайта удалит архив. Safari может удалить данные сайта; синхронизации между устройствами нет.</p><p><strong>Справочники.</strong> База материалов и официальный телефонный справочник загружаются с Secretworld. Введённый вами текст и номер к этим запросам не добавляются. Проверка не определяет владельца входящего вызова.</p><p><strong>Ограничения iPhone.</strong> Страница не читает SMS, звонки, SIM и установленные приложения. Она не блокирует рекламу в других программах и не отправляет push-уведомления.</p><p><strong>Технические запросы.</strong> Для работы PWA Safari загружает файлы сайта. Мы не включили аналитику и рекламные трекеры.</p>';
 document.addEventListener('click',async e=>{
  ...
  switch(b.dataset.action){
   ...
   case 'cat':{const quotes=['Красивые обещания тоже проходят проверку.','Я рядом. Спешить нас никто не просил.','Документы у подозрительных ссылок проверяем?','Слишком щедро? Давайте посмотрим, где подвох.'];$('#catQuote').textContent=quotes[Math.floor(Math.random()*quotes.length)];break;}
 ...
 $('#deviceHint').textContent=standalone?'Бегемот открыт с экрана «Домой».':ios?'Вы уже можете пользоваться страницей в Safari без установки. Для быстрого доступа добавьте её на экран «Домой».':'Эта отдельная веб-версия работает в браузере. На iPhone откройте её в Safari.';
 ...
 BegemotChat.ready.then(()=>{for(const m of BegemotChat.recent)bubble(m.role,m.role==='user'?'ВЫ':'БЕГЕМОТ',m.text);});const requested=new URLSearchParams(location.search).get('view');view(requested||'home',false);
})();
