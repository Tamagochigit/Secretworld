...
(() => {
 ...
 function view(v,scroll=true){
  if(!['check','network','chat','us','install'].includes(v))return;
  currentView=v;
  for(const name of ['check','network','chat','us','install'])$('#'+name+'Panel').hidden=name!==v;
 ...
 const help='<ol><li><strong>Сообщение или ссылка:</strong> вставьте в «Проверку». Разбор выполняется на устройстве; сайты не открываются.</li><li><strong>Звонок или iPhone:</strong> в разделе «iPhone» есть локальный справочник и короткий план ручной проверки.</li><li><strong>Разговор:</strong> без настройки работают встроенные подсказки. Свободный ИИ подключается отдельно вашим ключом.</li></ol><p>Бегемот помогает заметить признаки и выбрать следующий шаг, но не подтверждает безопасность отправителя или сайта.</p>';
